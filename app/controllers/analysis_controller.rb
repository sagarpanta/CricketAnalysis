		...
		###################################   bbr  bbb   dbx dbr dbb sql string variable definitions #############################################	
			...
			frequency_sc = '
							...
							(select s.grp1,s."over", s.ballsfaced,case when s.grp2<>s1.grp2 then 1 else 0 end as frequency_grp2, case when s.shottype between 28 and 43 or s.shottype in (7,10) then 1 else 0 end as mishits, case when s.shottype in (49,50,51,56) then 1 else 0 end as slugs
							 from (select grp1, grp2,"over", rank() over (partition by grp1, "over" order by ballnum) as ballnum, '+_metrics + ' from ' + scorecards +' s) s
								LEFT JOIN 
								(select grp1, grp2,"over",  rank() over (partition by grp1, "over" order by ballnum) as ballnum,'+_metrics + ' from ' + scorecards +' s) s1
