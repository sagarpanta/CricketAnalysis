class AnalysisController < ApplicationController
  ...
  def generate 
		...
		spellkey1 = params[:filters][:spkey]
		...
		combination = countrykey.product(formatkey, tournamentkey, venuekey, teamtypekey, teamkey, matchtypekey, coachkey, managerkey, playertypename, batsmankey, endkey, battingstylename, battingposition, inningkey, shottypekey, shotdirectionkey)

		combination1 = countrykey1.product(formatkey1, tournamentkey1, venuekey1, teamtypekey1, teamkey1, matchtypekey1, coachkey1, managerkey1, playertypename1, bowlerkey1, endkey1, bowlingtypename1, bowlingstylename1, bowlingposition1, inningkey1, linekey1, lengthkey1, bowlingsidekey1, spellkey1)

		countrykeys = formatkeys = tournamentkeys = venuekeys = teamtypekeys = teamkeys = matchtypekeys = coachkeys = managerkeys = playertypekeys = batsmankeys  = endkeys = battingstylekeys  = batpositionkeys  = inningkeys = shottypekeys = shotdirectionkeys='('
		countrykeys1 = formatkeys1 = tournamentkeys1 = venuekeys1 = teamtypekeys1 = teamkeys1 = matchtypekeys1 = coachkeys1 = managerkeys1 = playertypekeys1  = bowlerkeys1 = endkeys1  = bowlingstylekeys1 = bowlingtypekeys1  = bowlpositionkeys1 = inningkeys1 = linekeys1 = lengthkeys1 = bowlingsidekeys1 = spellkeys1 = '('
		...
		ismanagerempty = isplayertypeempty = isbatsmanempty  = isendempty = isbatstyleempty  = isbatposempty  =isinningempty = 0
		...
		ismanagerempty1 = isplayertypeempty1  = isbowlerempty1 = isendempty1  = isbowlstyleempty1 = isbowltypeempty1  = isbowlposempty1 =isinningempty1 = islengthkeyempty1 = 0
		...
		shotdirectionkeys  = isshotdirectionkeyempty==1? 'not in (-2)':'in '+shotdirectionkeys[0...-1] + ')'		
		...
		if analysiskey == 'Batting'
		   ...
		   where_always =' where s.clientkey = '+current_user.id.to_s + ' and s.matchkey >= '+ min_of_top_matchkeys.to_s + ' and ballnum between ' + ballnumber_betn		
			...
			pure_batting_group = ['batsman', 'bts', 'team', 'venue', 'dismissal','matchtype','teamtype' , 'tournament', 'coach', 'battingposition', 'direction', 'spell','manager', 'year', 'inning', 'format', 'country' ,'cr', 'pship' , 'shottype' , 'line', 'length']
			...
			not_required = ['matchtype', 'venue', 'tournament', 'format', 'battingposition', 'year', 'shottype' , 'line', 'length', 'direction', 'spell']
			...
			if !group1['matchtype'].nil? or !group2['matchtype'].nil? or !group2['countryagainst'].nil?  or matchtypekey[0] != ''
				build_query += match_part+' '+matchtype_part
				#match_part is excluded because match part is already include in the very first check
				build_query_match += matchtype_part
			end
...
=begin			
			if countrykey1[0] != '' and (group1 == 'countryagainst'  or group1 == 'teamagainst') and against_group.include? group1
				where_clause += where_countryagainstkeys1
			end
			if countrykey1[0] != '' and group1 != 'countryagainst' and group1 != 'teamagainst' and bowler_group.include? group1
				where_clause += where_countryagainstkeys1
			end
...
=begin
			if teamkey1[0] != '' and (group1 != 'teamagainst' or group2 != 'teamagainst')
				where_clause += where_teamagainstkeys1
			end
=end
			if teamtypekey1[0] != ''
...
=begin	
			...
			bowler_group = ['bowler' , 'bls' , 'bowlingposition' , 'bowlingtype', 'team', 'venue', 'matchtype', 'match', 'teamtype' , 'coach','manager', 'year', 'inning', 'format', 'country']
			...
			if (against_group.include? group1 and !against_group.include? group2) 
				...
				_group2['bowler'] = ',p1.fullname'
				_group2['bowlingtype'] = ',p1.bowlingtype'
				_group2['bowlingstyle'] = ',p1.bowlingstyle'
				_group1['cr'] = 'cr'
				_group1['pship'] = "case when currentstrikerkey<currentnonstrikerkey then p.fullname+'-'+p2.fullname else p2.fullname+'-'+p.fullname end"
			end
=end
		   ...
		   where_coachkeys1 =' and tm1.coachkey '+coachkeys1
		   where_managerkeys1 =' and tm1.managerkey '+managerkeys1
		   where_matchtypekeys1 =' and mt1.id '+matchtypekeys1
		   where_formatkeys1 =' and s.formatkey '+formatkeys1
		   where_linekeys1 = ' and l.id '+linekeys1
		   where_lengthkeys1 = ' and l.id '+lengthkeys1
		   where_spellkeys1 = ' and s.spell '+spellkeys1
		   where_bowlingsidekeys1 = ' and s.side '+bowlinsidekeys1
		   ...
		   where_always =' where s.clientkey = '+current_user.id.to_s + ' and s.matchkey >= '+ min_of_top_matchkeys.to_s		
			...
			pure_bowling_group = ['bowler' , 'bls'  , 'bowlingtype', 'team', 'venue', 'dismissal','matchtype', 'match','teamtype' , 'tournament', 'coach', 'bowlingposition', 'manager', 'year', 'inning', 'format', 'country' ,'cr', 'pship' , 'shottype' , 'line', 'length','side', 'direction', 'spell']
			#the followings are the part of batting group, but they are not requirement for adding batting_part...ie if they belong to group2
			not_required = ['matchtype', 'venue', 'tournament', 'format', 'bowlingposition', 'year', 'shottype' , 'line', 'length' ,'side','direction', 'spell']
			
			if pure_bowling_group.include? group1 or  teamkey1[0] != '' or teamtypekey1[0] != '' or coachkey1[0] != '' or managerkey1[0] != ''
				build_query_match = bowler_part + teamagainst_part + teamtypeagainst_part + coachagainst_part + manageragainst_part 
			elsif (pure_bowling_group.include? group2 or bowlerkey1[0] != '' or countrykey1[0] != '' or playertypename1[0] != '' or bowlingstylename1[0] != '')
				if !not_required.include? group2
					build_query_match = bowler_part
				end
				if (['team', 'coach', 'manager', 'teamtype'].include? group2 or  teamkey1[0] != '' or teamtypekey1[0] != '' or coachkey1[0] != '' or managerkey1[0] != '')
					build_query_match += teamagainst_part + teamtypeagainst_part + coachagainst_part + manageragainst_part 
				end
			end
			
			if against_group.include? group1 or teamkey[0] != '' or managerkey[0] != '' or coachkey[0] != '' or teamtypekey[0] != ''
				build_query_match += batsman_part + team_part + teamtype_part + coach_part + manager_part 
			elsif against_group.include? group2 or batsmankey[0] != '' or battingstylename[0] != '' or playertypename[0] != ''  or countrykey[0] != '' 
				if group2 != 'battingposition'
					build_query_match += batting_part 
				end
				if !group2['teamagainst'].nil? or teamkey[0] != '' or managerkey[0] != '' or coachkey[0] != '' or teamtypekey[0] != ''
					build_query_match += teamagainst_part + teamtypeagainst_part + coachagainst_part + manageragainst_part
				end
			end			
			...
			if !group1['match'].nil? or !group2['match'].nil?
				build_query+= match_part
				build_query_match += match_part
			end
			if !group1['matchtype'].nil? or !group2['matchtype'].nil? or !group2['countryagainst'].nil?  or matchtypekey1[0] != ''
				build_query += match_part+' '+matchtype_part
				build_query_match+= matchtype_part
			end
...
=begin
			...
			#followings are the groups/filters...when applied, players p table need to be joined with scorecards s  so that team, country and so on can then join
			batsman_group = ['batsman', 'bts', 'team', 'venue', 'matchtype', 'match','teamtype' , 'coach', 'manager', 'cr', 'pship']
			
			#follwings are the pure bowler group which are used to monitor filters in the bowling side. 
			#such as when country is selected on the right hand side and one of the groups is in the group, then the filter will apply to the bowling group rather than the batting group
			bowler_group = ['bowler' , 'bls' , 'bowlingposition' , 'bowlingtype']	

			#pure_batting_group = ['batsman', 'bts', 'team', 'venue', 'dismissal','matchtype', 'match','teamtype' , 'tournament', 'coach', 'battingposition', 'manager', 'year', 'inning', 'format', 'country' ,'cr', 'pship' , 'shottype' , 'line', 'length']
			...
			#check only for teams, coaches, managers and teamtypes from the right hand side, because the bowler filter has added the necessary joins before the current filter adds the team against joins
			if !group1['teamagainst'].nil? or !group2['teamagainst'].nil? or teamkey1[0] != '' or managerkey1[0] != '' or coachkey1[0] != '' or teamtypekey1[0] != ''
				build_query += teamagainst_part + teamtypeagainst_part + coachagainst_part + manageragainst_part
			end
			#check only for country from the right hand side, because the bowler filter has added the necessary joins before the current filter adds the country against joins
			if  !group1['countryagainst'].nil? or !group2['countryagainst'].nil?  or countrykey1[0] != ''
		...
		###################################   bbr  bbb   dbx dbr dbb sql string variable definitions #############################################	
			bbr = '
				...
				(
					...
					from
					(
					select rank() over (order by  '+_group1[group1]+(!_group2[group2].nil? ? _group2[group2]:'')+' , ballnum) as _rank, 
